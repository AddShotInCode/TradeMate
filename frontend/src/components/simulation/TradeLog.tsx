              ...
              {tradeLogs.map((log) => (
                <Fragment key={log.id}>
                  ...
                  >
                    ...
                    <td
                      className={`px-4 py-2 ${log.side === "BUY" ? "text-red-500" : "text-blue-500"}`}
                    >
                      {log.side}
                    </td>
                    ...
                    <td
                      className={`px-4 py-2 text-right font-mono font-bold ${log.pnl >= 0 ? "text-red-500" : "text-blue-500"}`}
                    >
                      {log.pnl !== 0
                        ? `${log.pnl > 0 ? "+" : ""}₩${Math.round(log.pnl).toLocaleString("ko-KR")}`
                        : "-"}
                    </td>
                  </tr>
                  ...
                  <tr
                    id={`details-${log.id}`}
                    className="hidden bg-[#151c24]/50 border-b border-[#283039]"
                  >
                    <td colSpan={6} className="px-4 py-3">
                      <div className="flex gap-4 text-xs">
                        <span className="text-slate-500 font-bold whitespace-nowrap pt-1">
                          진입 근거:
                        </span>
...
function OpenPositionsTable() {
  const { positions, currentPrice } = useSimulationStore();
    ...
    <table className="w-full text-left border-collapse">
      <thead className="bg-[#151c24] text-slate-400 text-xs uppercase sticky top-0 z-10">
        <tr>
          ...
          <th className="px-4 py-3 font-medium">수량 (Qty)</th>
          <th className="px-4 py-3 font-medium text-right">미실현 손익 (Unrealized P&L)</th>
        </tr>
      </thead>
      <tbody className="text-sm text-slate-300 divide-y divide-[#283039]">
        {positions.map((pos, idx) => {
          ...
          return (
            <tr key={idx} className="hover:bg-[#283039]/50">
              ...
              <td
                className={`px-4 py-2 font-mono font-bold text-right ${pnl > 0 ? "text-red-500" : pnl < 0 ? "text-blue-500" : "text-slate-500"}`}
              >
                {pnl > 0 ? "+" : pnl < 0 ? "-" : ""}₩
