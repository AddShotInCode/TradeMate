...
export default function InquiryChart() {
  ...
  const seriesRef = useRef<ISeriesApi<"Candlestick"> | null>(null);
    ...
    const [crosshairLegend, setCrosshairLegend] = useState<{ open: number; high: number; low: number; close: number; date: string } | null>(null);
  ...
  const currentDisplay = crosshairLegend || (lastCandle ? {
            ...
            {/* Legend Overlay */}
                        {currentDisplay && displayData.length > 0 && (
                                <div className="absolute top-12 left-3 z-10 flex items-center gap-3 text-xs font-mono bg-[#111418]/80 p-2 rounded border border-[#283039] backdrop-blur-sm">
                                        <span className={`${currentDisplay.close > currentDisplay.open ? 'text-red-500' : 'text-blue-500'}`}>
                                        ...
                                        <div className="relative">
                                            ...
                                            >
                                                ...
                                                <div className="pointer-events-none absolute left-full top-1/2 ml-2 hidden w-[180px] -translate-y-1/2 rounded-md border border-[#283039] bg-[#0b0f14]/95 px-2 py-1.5 text-[11px] leading-snug text-slate-200 shadow-lg group-hover:block">
                                                    <div><span className="font-semibold">O</span>: 시가 (Open)</div>
                                                    <div><span className="font-semibold">H</span>: 고가 (High)</div>
                                                    <div><span className="font-semibold">L</span>: 저가 (Low)</div>
                                                    <div><span className="font-semibold">C</span>: 종가 (Close)</div>
                                                </div>
                                            </button>
                                        </div>
                                </div>
