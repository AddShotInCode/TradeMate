...
export default function FinancialStatementModal({ 
  ...
  // Generate Year Options (up to Sim Year)
  const renderYearOptions = () => {
      if (!currentSimDateStr) return null;
      const simYear = new Date(currentSimDateStr).getFullYear();
      const years = [];
      for (let y = simYear; y >= 2015; y--) {
          years.push(y);
      }
      return years.map(y => <SelectItem key={y} value={String(y)}>{y}년</SelectItem>);
  };
            ...
            <div className="min-h-[200px] border border-slate-700 rounded-lg p-4 bg-[#101922]">
                {items.length > 0 ? (
                    <div className="space-y-2">
                        {items.map((item, idx) => (
                            <div key={idx} className="flex items-center justify-between p-3 bg-[#283039] rounded hover:bg-[#3b4754] transition-colors">
                                <div className="flex flex-col">
                                    <span className="font-medium text-blue-400">
                                        {year}년 {quarter}분기 보고서 ({item.rcept_dt.substring(0,4)}.{item.rcept_dt.substring(4,6)}.{item.rcept_dt.substring(6,8)})
                                    </span>
                    ...
                    <div className="h-full flex flex-col items-center justify-center text-slate-500 gap-2">
                        ...
                        <span className="text-xs text-slate-600 mt-2">
                            * 시뮬레이션 시점({currentSimDateStr}) 이전에<br/>공시된 리포트만 조회 가능합니다.
                        </span>
                    </div>
                )}
            </div>
