    ...
    <div className="grid grid-cols-3 gap-4 p-4 bg-slate-50 dark:bg-[#151c24] border-b border-slate-200 dark:border-[#283039]">
      ...
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-center gap-2">
          <p
            className={`${unrealizedPnl > 0 ? "text-red-500" : unrealizedPnl < 0 ? "text-blue-500" : "text-slate-500"} text-lg font-bold font-mono`}
          >
            {unrealizedPnl > 0 ? "+" : unrealizedPnl < 0 ? "-" : ""}₩
            {Math.abs(Math.round(unrealizedPnl)).toLocaleString("ko-KR")}
          </p>
        </div>
      </div>
      <div className="flex flex-col gap-1">
