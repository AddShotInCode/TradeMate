...
export default function SimulationChart() {
  ...
  const { data, isLoading, currentTimeIndex, setCurrentPrice, currentPrice, loadData, error } = useSimulationStore();
    ...
    const initChart = (width: number, height: number) => {
        ...
        chart.subscribeCrosshairMove((param) => {
             if (!param.point || !param.time || param.point.x < 0 || param.point.x > chart.timeScale().width()) {
                 const visibleData = dataRef.current.slice(0, Math.min(currentIndexRef.current + 1, dataRef.current.length));
                 if (visibleData.length > 0) {
