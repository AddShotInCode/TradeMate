...
export default function OrderForm() {
  const { placeOrder, sellPosition, currentPrice, balance, positions } = useSimulationStore(); // Add positions, sellPosition
  ...
  const estimatedLoss = (activeTab === 'BUY' && stopLoss !== '' && currentPrice > 0) 
    ? (Number(stopLoss) - currentPrice) * qty 
    : 0;
  const estimatedProfit = (activeTab === 'BUY' && target !== '' && currentPrice > 0)
